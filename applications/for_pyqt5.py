...
def application():  # создание основной функции
    app = QApplication(sys.argv)  # создание нового объекта от класса QApplication. sys.argv - настройки компьютера, на
    # котором запускается приложение
    window = Window()  # окно внутри приложения

    window.show()  # метод показывает окно
    sys.exit(app.exec_())  # программа всегда будет завершаться корректно
